...
class Query:
    def __init__(self, restquery, database, pid):
...
class DruidClient:
    ...
    def druid_query(self, querysql, pid):  # 数据源是固定的，但是table可能有变化
        query=Query(self.datasource,querysql,pid)
        query.do_query()
